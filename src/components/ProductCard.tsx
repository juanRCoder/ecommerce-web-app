...
export const ProductCard = ({ product, cartMode = false }: ProductCardProps) => {
  ...
  return (
    <div className={`flex relative outline outline-[#29292930] rounded-[10px]
      ${cartMode ? 'flex-row max-[280px]:flex-col' : 'flex-col'}`}
    >
      <div className={`flex flex-col gap-2 ${cartMode ? 'py-2 items-center justify-center' : ''}`}>
        <div className={`flex items-center justify-center ${cartMode ? '!w-[120px]' : ''}`}>
          <img
      ...
      <div className={`flex flex-col p-2 ${cartMode ? 'gap-2 justify-center mr-12' : ''}`}>
        ...
        {cartMode ? (
          <div className='inline-flex items-center justify-between rounded-[5px] w-24'>
            <span onClick={()=>updateProductFromCart(product.id, product.quantity - 1)} className='bg-[#208572] rounded-[5px] p-1.5 flex-1 grid place-items-center cursor-pointer'>
              <Minus size={16} />
            </span>
      ...
      {cartMode && (
        <div onClick={()=>removeProduct(product.id)} className='absolute top-2 right-2 outline-1 bg-[#a7273833] p-2 cursor-pointer rounded-full'>
